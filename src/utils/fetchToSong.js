export const fetchToSong = async () => {
  const artists = await fetch(
    'http://ws.audioscrobbler.com//2.0/?method=chart.gettopartists&api_key=8d0e1a9d07a2dc19acb1ca07f031a4a6&limit=50&format=json'
  );
  const json = await artists.json();
  const artist = await fetch(
    `https://deezerdevs-deezer.p.rapidapi.com/search?q=${
      json.artists.artist[Math.floor(Math.random() * json.artists.artist.length - 1)].name
    }`,
    {
      method: 'GET',
      headers: {
